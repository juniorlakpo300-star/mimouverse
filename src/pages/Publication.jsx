...
const BOOK_GENRES = [
  'Roman', 'Nouvelle', 'Conte', 'Poésie', 'Théâtre', 'Fable', 'Épopée', 'Légende', 'Mythe',
  'Biographie', 'Autobiographie', 'Mémoires', 'Journal intime', 'Correspondance', 'Essai',
  'Discours', 'Chronique', 'Récit', 'Satire', 'Comédie', 'Tragédie', 'Drame', 'Aventure',
  'Romance', 'Policier', 'Thriller', 'Fantastique', 'Fantasy', 'Science-fiction', 'Historique',
  'Dystopie', 'Utopie', 'Jeunesse', 'Philosophique', 'Éducatif', 'Développement personnel',
  'Littérature africaine', 'Littérature classique', 'Littérature contemporaine',
]

const MANGA_GENRES = [
  'Shōnen', 'Shōjo', 'Seinen', 'Josei', 'Kodomo', 'Isekai', 'Réincarnation', 'Action', 'Aventure',
  'Fantasy', 'Fantastique', 'Romance', 'Comédie', 'Drame', 'Horreur', 'Thriller', 'Mystère',
  'Policier', 'Psychologique', 'Science-fiction', 'Surnaturel', 'Sport', 'Mecha', 'Arts martiaux',
  'School Life', 'Slice of Life', 'Magical Girl', 'Super-pouvoirs', 'Musique', 'Cuisine', 'Jeux',
  'Historique', 'Samouraï', 'Arts du spectacle', 'Comédie romantique', 'Dark Fantasy',
]
...
export default function Publication() {
  ...
  const genres = form.type === 'manga' ? MANGA_GENRES : BOOK_GENRES
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }))
  const changeType = (type) => setForm((current) => ({ ...current, type, category: '' }))
  ...
  const publish = async (event) => {
    ...
    if (!form.category) return setMessage({ type: 'error', text: 'Sélectionne un genre.' })
    ...
    <main className="publication-page">
      ...
      <div className="publication-wrap">
        ...
        <form className="publication-card" onSubmit={publish}>
          ...
          <div className="publication-type"><button type="button" className={form.type === 'books' ? 'active' : ''} onClick={() => changeType('books')}><BookOpen size={18} /> Livre</button><button type="button" className={form.type === 'manga' ? 'active' : ''} onClick={() => changeType('manga')}><BookMarked size={18} /> Manga</button></div>
          <div className="publication-grid">
            ...
            <div className="publication-field"><label>Auteur / Mangaka</label><input value={form.author} onChange={(e) => update('author', e.target.value)} placeholder={form.type === 'manga' ? 'Nom du mangaka' : 'Nom de l’auteur'} /></div>
            <div className="publication-field"><label>Genre *</label><select value={form.category} onChange={(e) => update('category', e.target.value)}><option value="">Choisir un genre</option>{genres.map((genre) => <option key={genre} value={genre}>{genre}</option>)}</select></div>
