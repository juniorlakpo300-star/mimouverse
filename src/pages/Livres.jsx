import { BookOpen, Heart, Search, ArrowRight, Loader2, Quote, X, Maximize2 } from 'lucide-react'
    ...
    {reader && <div className="book-reader-overlay" role="dialog" aria-modal="true" aria-label={`Lecture de ${reader.title}`}>
      <div className="book-reader-shell">
        <header className="book-reader-header">
          <div><strong>{reader.title}</strong><span>{reader.author}</span></div>
          <div className="book-reader-actions"><a href={reader.file} target="_blank" rel="noopener noreferrer" aria-label="Ouvrir le lecteur PDF en plein écran"><Maximize2 size={18}/></a><button onClick={() => setReader(null)} aria-label="Fermer la lecture"><X size={20}/></button></div>
        </header>
