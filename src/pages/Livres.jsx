import { BookOpen, Heart, Search, ArrowRight, Loader2, Quote } from 'lucide-react'
...
async function openFile(file) { if (file) window.open(file, '_blank', 'noopener,noreferrer') }
...
export default function Livres() {
  ...
  useEffect(() => { let active = true; const load = async () => { if (!supabase) { setError('Supabase n’est pas configuré.'); setLoading(false); return }; const { data, error: queryError } = await supabase.from('books').select('*').order('created_at', { ascending: false }); if (!active) return; if (queryError) setError(queryError.message); else setBooks((data || []).map(normalizeBook)); setLoading(false) }; load(); return () => { active = false } }, [])
