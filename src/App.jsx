    ...
    <main>
      ...
      <section className="section">
        <div className="section-heading">
          <div><span className="kicker">Tout au même endroit</span><h2>Un univers, plusieurs façons de lire</h2></div>
          <p>Une base solide, pensée pour évoluer avec tes lecteurs.</p>
        </div>
        <div className="feature-grid">
          {features.map(({ to, icon: Icon, title, text }) => (
            <Link className="feature-card" to={to} key={to}>
              <div className="icon-box"><Icon size={23} /></div><h3>{title}</h3><p>{text}</p><span>Découvrir →</span>
            </Link>
          ))}
        </div>
      </section>
...
export default function App() {
  return <div className="app">
    ...
    <Routes>
      ...
      <Route path="/livres" element={<Placeholder title="Bibliothèque" text="La bibliothèque de livres arrive ici, avec recherche, catégories, favoris et lecture." />} />
      ...
      <Route path="/connexion" element={<Placeholder title="Connexion" text="L’authentification des lecteurs sera ajoutée dans la prochaine étape." />} />
    </Routes>
